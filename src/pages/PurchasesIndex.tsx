...
const PurchaseIndex = () => {
  ...
  useEffect(() => {
    const fetchAllCustomerSales = async () => {
      ...
      // 1. Decode the phone number to remove URL characters like %20 or %2B
      const decodedPhone = decodeURIComponent(phone);

      try {
        // 2. Use both ?search= and ?phone= to ensure the Django backend catches it
        const response = await fetch(`${API_BASE_URL}/sales/?search=${encodeURIComponent(decodedPhone)}&phone=${encodeURIComponent(decodedPhone)}`, {
        ...
        // 3. Relaxed Frontend Filter: We trust the backend's search results. 
        // If we strictly check === it breaks if one has spaces and the other doesn't.
      ...
      <div className="max-w-5xl mx-auto space-y-6 pb-20 pt-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-500 bg-slate-900 rounded-2xl border border-dashed border-slate-800">
            No purchase records found for {decodeURIComponent(phone || "")}.
          </div>
        ) : (
          <>
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 flex justify-between items-center">
               <div>
                 ...
                 <p className="text-lg text-emerald-500 font-mono mt-1 flex items-center gap-2"><Phone className="w-4 h-4"/> {decodeURIComponent(phone || "")}</p>
               </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {sales.map((sale) => (
                <div 
                  ...
                  onClick={() => navigate(`/sales/${encodeURIComponent(phone || "")}/${sale.invoice_number}`)} 
                ...
                >
                  ...
                  <div className="flex items-center gap-4 md:w-1/4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-sm mt-1">
                        Date: {sale.date_sold || sale.created_at ? new Date(sale.date_sold || sale.created_at).toLocaleDateString() : "N/A"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 w-full border-y md:border-y-0 md:border-l border-slate-800 py-4 md:py-0 md:pl-6 flex flex-col gap-3">
                    {/* 4. Safety Check: Only map if sale.items is actually a valid array */}
